  ...
  >
    ...
    <div class="max-w-sm mb-3">
      ...
      <textarea
        ...
        rows="3"
        placeholder="A collection of the lunar star names..."
      ></textarea>
    </div>
...
const createNewPlaylist = async () => {
  ...
  playlist
    ...
    .then(async () => {
      await message(
        `${_.capitalize(name.value)} playlist created successfully`,
        { title: "Echo", kind: "info" }
      );
    })
    .catch(async (err: unknown) => {
      await message(
        `Error creating ${_.capitalize(name.value)} due to ${(err)}`,
        { title: "Echo", kind: "info" }
      );
    });
};
